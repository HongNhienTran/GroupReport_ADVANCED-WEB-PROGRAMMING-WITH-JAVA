...
export default function ProductCard({
    ...
    return (
        <div className="bg-white p-3.5 sm:p-4 rounded-xl border border-slate-200/70 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between h-[380px] group relative select-none">
            ...
            {discount && discount > 0 ? (
                <span className="absolute top-2.5 left-2.5 z-10 bg-red-500 text-white text-[10px] font-black px-2 py-0.5 rounded shadow-sm tracking-wide">
                    -{discount}%
                </span>
            ) : null}
            ...
            <button
                ...
                className="absolute top-2.5 right-2.5 z-10 w-7 h-7 rounded-full bg-white/90 hover:bg-white shadow-sm flex items-center justify-center transition-all duration-200 hover:scale-110 active:scale-90 cursor-pointer"
            >
                <Heart
            ...
            <div className="flex-1 flex flex-col">
                {/* 3. Khung ảnh sản phẩm (Bo góc tự nhiên rounded-lg) */}
                <div className="w-full h-40 bg-gray-50 rounded-lg mb-2.5 flex items-center justify-center overflow-hidden relative flex-shrink-0">
                    {imageUrl?.startsWith('http') || imageUrl?.startsWith('/') ? (
                        <img
                            src={imageUrl}
                            alt={name}
                            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300 select-none pointer-events-none rounded-lg"
                        />
                    ) : (
                    ...
                    {dietaryTags && dietaryTags.length > 0 && (
                        <span className="absolute bottom-1.5 left-1.5 text-[9px] font-bold px-1.5 py-0.5 rounded bg-white/90 text-emerald-800 shadow-xs border border-emerald-100">
                            {dietaryTags[0]}
                        </span>
                    )}
                </div>
                ...
                <h3 className="font-bold text-slate-800 text-xs sm:text-sm text-left h-9 line-clamp-2 leading-snug tracking-wide group-hover:text-emerald-700 transition-colors flex items-start flex-shrink-0">
                    {name}
                </h3>
            ...
            <div className="mt-auto pt-2 border-t border-slate-100 flex items-center justify-between flex-shrink-0">
                ...
                <button
                    ...
                    className="w-8 h-8 rounded-lg bg-emerald-50 hover:bg-emerald-600 text-emerald-700 hover:text-white font-bold transition-all duration-200 shadow-xs hover:shadow-sm flex items-center justify-center cursor-pointer active:scale-90"
                >
